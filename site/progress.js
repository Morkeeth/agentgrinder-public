  ...
  async function historyView() {
    ...
    try {
      ...
      function render() {
        ...
        $('history-list').innerHTML=filtered.map(runTile).join('')||`<div class="panel reply-form"><h2>${loaded.length?'No matching runs':'Your first run starts here'}</h2><p>${loaded.length?'Change the filters or load older runs.':'Record a local session, preview what leaves your machine, then choose its audience.'}</p>${loaded.length?'':'<a href="/?post">Post my first run</a>'}</div>`;
      }
