...
const fixture={...sample,id,title:'sample-project: a two-hour probe',started_at:sample.started,prompts:sample.turns_typed,visibility:'public',profiles:{github_handle:'fixture-builder'}};
assert.deepEqual(await readPublic(id,async(url,options)=>{assert.equal(new URL(url).searchParams.get('visibility'),'eq.public');assert(!new URL(url).searchParams.get('select').includes('note'));assert(new URL(url).searchParams.get('select').includes('github_handle,handle,display_name'));assert(new URL(url).searchParams.get('select').includes('wall_time_s'));assert(new URL(url).searchParams.get('select').includes('route'));assert.equal(options.cache,'no-store');assert.equal(options.headers['Accept-Profile'],'strava');return{ok:true,json:async()=>[fixture]}}),fixture);
