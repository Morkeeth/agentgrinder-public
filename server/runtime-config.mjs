...
export function runtimeConfig(env=process.env) {
 ...
 if(env.VERCEL && (!env.AGENTGRINDER_SUPABASE_URL||!env.AGENTGRINDER_SUPABASE_ANON_KEY||!env.STRAVA_ORIGIN)) throw new Error('Production requires the Strava origin and public Supabase configuration');
 ...
 return {SB_URL:origin(url,'Supabase URL'),SB_KEY:key,SB_SCHEMA:'strava',ORIGIN:origin(env.STRAVA_ORIGIN||'http://localhost:8000','Strava URL')};
}
