...
// existed, and the page must not tell those apart. The one link goes to /?run=id, where a
...
export function neutralHtml(id){const safe=encodeURIComponent(id),image=origin+'/api/run?id='+safe+'&image=1',url=origin+'/r/'+safe,title=esc(`This run is private on ${BRAND}`),description='Sign in and open the shared run link to check your access.';
 return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title}</title><meta name="robots" content="noindex"><meta property="og:type" content="article"><meta property="og:title" content="${title}"><meta property="og:description" content="${description}"><meta property="og:url" content="${url}"><meta property="og:image" content="${image}"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${title}"><meta name="twitter:description" content="${description}"><meta name="twitter:image" content="${image}"><style>body{background:#f8f8f6;color:#111;font:18px system-ui;margin:0;padding:32px}main{max-width:900px;margin:40px auto}img{width:100%;height:auto;border:1px solid #ddd}a{color:#123cff}h1{font-size:clamp(28px,5vw,48px);line-height:1.1}a.open{display:inline-block;background:#123cff;color:white;text-decoration:none;padding:16px 24px;margin:24px 0}</style></head><body><main><a href="/">${BRAND}</a><h1>${title}</h1><p>${description}</p><img src="${image.replaceAll('&','&amp;')}" alt="${title}" width="1200" height="630"><a class="open" href="/?run=${safe}">Sign in and open the run</a></main></body></html>`;
}
