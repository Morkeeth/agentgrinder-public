...
Start with GitHub and X linked to one internal identity, with separate Strava onboarding/profile. The existing github_handle and GitHub metadata assumptions need a compatibility migration before enabling X. Do not match account ownership by public handles. Origin is Cursor's code forge. Its [official API](https://cursor.com/docs/api/origin) documents app installation consent, signed installation receipts and scoped repository tokens. Treat it as a separate repository connection after Strava sign-in. Register an Origin app/public signing key and exact callback, request selected-repository metadata access, verify receipt signature/issuer/audience/expiry/state, and bind the installation to the signed-in Strava profile. Do not use the receipt's installer email to merge accounts. App registration and a real installation remain pending; no Origin login button is enabled.
