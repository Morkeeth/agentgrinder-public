...
Boards, 25 September: `/?boards` is computed in the browser from public runs only. Time on a run is `wall_time_s` when measured, else `duration_s`, the same precedence the feed card uses. A Marathon is a public run of 3 hours or more, all time. The weekly lanes (Monday 00:00 in the reader's time zone) are: longest ghost run (the feed card's own ghost rule), most tool calls, and the honest failure (most tool calls with zero files changed, or zero commits when files are unknown). An empty lane says so; no sample fills it. A profile carries a 52-week heatmap of the runs the viewer can see.
