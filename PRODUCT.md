...
Connect ruling, 23 September: connecting a device is one act, and then it syncs. A device asks for a code (RFC 8628, the flow `gh auth login` uses), a signed-in person approves that code once on a phone at `/?pair=<code>`, and the device keeps its own credential; nobody copies a secret between two machines. The credential is private draft and publish only, it slides 90 days forward every time the device uploads a run, and Connections lists each device with when it last synced, marks a device with no run for over a week, and revokes it in one press. A code lives 900 seconds, is answered no faster than every 5 seconds, can be approved once, and a denied or expired code never becomes a credential. The pairing funnel records five steps per pairing — `connect_started`, `code_issued`, `code_approved`, `token_claimed`, `first_run_received` — plus one terminal step when a pairing is denied or expires, and its table has no column that could hold content. Uploads still carry counts and allowlisted fields only: a transcript's text, prompts and home directory never leave the machine.
...
- The harness side of device pairing. `POST /api/connect/device` and `POST /api/connect/token` exist and the approve page works, but no shipped command drives them yet: a device pairs by calling the two endpoints itself. A `connect device` command for the local CLI and hook is the next slice.
