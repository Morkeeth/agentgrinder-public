-- 008: declared outcome receipts on a run, so a card can say which repo, what shipped and where to look.
--
-- Oscar, 20 September: a shareable run should show repos, features shipped, receipts and a demo or
-- image. Nothing in the schema held any of that, and site/sharing.js renders output_url which the
-- upload allowlist never accepted, so an agent could not set it.
--
-- These five fields are DECLARED by the uploader, never measured. The card keeps them under their own
-- heading, away from counts and the ridge, so a link can never read as evidence that work happened.
-- Validation is server side: https only, bounded length and count, repository links restricted to
-- known code hosts, images restricted to raster extensions so no SVG is rendered.
-- Strava schema only. Applies on top of 006 and 007.

begin;

alter table strava.runs add column if not exists repo_url text;
alter table strava.runs add column if not exists receipts jsonb;
alter table strava.runs add column if not exists shipped jsonb;
alter table strava.runs add column if not exists artifact_url text;
alter table strava.runs add column if not exists image_url text;
do $$ begin
 if not exists(select 1 from pg_constraint where conname='runs_outcome_shape_check') then
  alter table strava.runs add constraint runs_outcome_shape_check check(
   (receipts is null or (jsonb_typeof(receipts)='array' and jsonb_array_length(receipts)<=5))
   and (shipped is null or (jsonb_typeof(shipped)='array' and jsonb_array_length(shipped)<=5))
   and (repo_url is null or repo_url like 'https://%')
   and (artifact_url is null or artifact_url like 'https://%')
   and (image_url is null or image_url like 'https://%'));
 end if;
end $$;

-- One https link, bounded, with nothing that could break out of an href.
create or replace function strava.grinder_is_safe_url(u text)
 returns boolean
 language sql
 immutable
 set search_path to 'strava', 'pg_temp'
as $function$
 select u is not null and u like 'https://%' and length(u) between 12 and 300
  and u !~ '[[:space:]<>"''\\\\]' and strpos(lower(u),'javascript:')=0
$function$;
...
begin
 ...
 for field,value in select * from jsonb_each(payload) loop
  if not field=any(array['title','project','harness','turns_typed','duration_s','tool_calls','files_touched','commits','claims','claims_verified','artifacts_produced','started','visibility','rhythm','route','schema_version','measurement_revision','baseline_revision','trace_basis','note','run_id','body','reason','question_id',
    ...
    -- Declared outcome receipts, added 2026-09-20. Links and short lines the uploader states.
    -- They are never measurements: the card shows them under a separate heading.
    'repo_url','receipts','shipped','artifact_url','image_url']) then raise exception 'Unsupported public field: %',field; end if;
 ...
 -- Declared outcome receipts. Strings only, https only, bounded in count and length. A link the
 -- uploader states is not evidence the work happened; the card labels them as declared.
  ...
  if action='draft' then
   ...
   if not existing then
    ...
    values(actor.owner_id,left(coalesce(payload->>'title','Agent grind'),200),left(payload->>'project',200),left(payload->>'harness',100),
     ...
     nullif(payload->>'artifact_url',''),nullif(payload->>'image_url',''))
...
revoke all on function strava.grinder_is_safe_url(text) from public, anon, authenticated;
